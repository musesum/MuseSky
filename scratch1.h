...
panel {
    ...
    cell {
        ...
        slide {
            ...
            controls {
                hide { type "panelx", title "hide", frame (x 0, y 0, w 40, h 40), icon "icon.thumb.X.png", value (0:1) }
                ruleOn { type "panelon", title "Active", frame (x 252, y 6, w 48, h 32), icon "icon.cell.slide.png",
                    value (0:1)
                    >> panel.cell˚ruleOn.value 0
                    >> panel.cell.speed.restart
                    >> sky.shader.cellSlide.on,
                    lag 0 }
    ...
    shader {
        ...
        tile {
            ...
            controls {
                hide { type "panelx", title "hide", frame (x 0, y 0, w 40, h 40), icon "icon.thumb.X.png", value (0:1) }
                tileOn { type "panelon", title "Active", frame (x 174, y 6, w 48, h 32), icon "icon.shader.tile.png", value (0:1)
                    user >> repeatBox.value (x 0, y 0)  { x, y } lag 0 }
                mirrorBox { type "box", title "Mirror", frame (x 10, y 80, w 80, h 80), radius 10, tap2 (1, 1), lag 0, user (0:1=1), value (0, 0) (0:1) >> sky.shader.render.buffer.mirror }
                repeatBox { type "box", title "Repeat", frame (x 100, y 40, w 120, h 120), radius 10, tap2 (-1, -1), lag 0,
                    user (0:1=1)
                    >> tileOn.value 1
                    >> value (0:1,0:1)
                    >> sky.shader.render.buffer.repeat } } } } }
