√ {
    sky {
        ...
        input {
            azimuth(x -0.2:0.2, y -0.2:0.2) force(0:0.5) >> sky.draw.brush.size  accel(x -0.3:0.3, y -0.3:0.3, z -0.3:0.3) { on(0:1) } radius(1:92=9) tilt(0:1) }
        draw {
            screen { fillZero(0) fillOne(-1) }
            brush { type "dot" size(1:64 =10 ) press(0:1) index(1:255 = 127) }
        ...
        shader {
            ...
            cellZhabatinski { type "compute" file "cell.zhabatinski.metal" on(0:1)
                buffer { version(0:1) bits(2:4 = 2) } repeat(11) }
